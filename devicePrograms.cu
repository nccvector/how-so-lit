...
#include "common/cudaMathHelper.h"

#include <vector_functions.h>

/*! launch parameters in constant memory, filled in by optix upon
      optixLaunch (this gets filled in from the buffer
 * we pass to
      optixLaunch) */
...
extern "C" __global__ void __raygen__renderFrame() {
  ...
  const float3 color = ( 1 - t ) * make_float3( 1, 1, 1 ) + t * make_float3( .3, .5, 1 );
  const int3   rgb   = make_int3( color.x * 255, color.y * 255, color.z * 255 );

  // convert to 32-bit rgba value (we explicitly set alpha to 0xff
  // to make stb_image_write happy ...
  const uint32_t rgba = 0xff000000 | ( rgb.x << 0 ) | ( rgb.y << 8 ) | ( rgb.z << 16 );
